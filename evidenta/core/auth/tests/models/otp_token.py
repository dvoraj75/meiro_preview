...
from evidenta.core.auth.models import OTPToken
from evidenta.core.user.models import User


@freeze_time(timezone.now())
@pytest.mark.django_db
def test_create_token_should_successfully_pass(random_otp_token: str, admin: User) -> None:
    OTPToken.objects.create(
        token=random_otp_token,
        user=admin,
    ...
    assert_count(qs, 1)
    token = qs.first()
    assert_equal(token.token, random_otp_token)
...
def test_token_is_valid(random_otp_token: str, admin: User, expires_at: datetime, is_valid: bool) -> None:
    OTPToken.objects.create(
        token=random_otp_token,
        user=admin,
